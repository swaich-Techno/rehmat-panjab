...
export default async function Page(){
  ...
  return <main id="main-content" className="admin-page"><p className="eyebrow">Private super-admin review</p><h1>Store readiness.</h1><p>Commerce remains {COMMERCE_ENABLED?"enabled":"disabled"}. Razorpay and Shiprocket must remain inactive until every operational check is verified.</p>
    ...
    <section className="admin-empty"><h2>Provider status</h2><p><strong>Razorpay:</strong> {status.get("razorpay")??"pending"} · <strong>Shiprocket:</strong> {status.get("shiprocket")??"pending"}</p>{razorpayLink&&<p>Verified Razorpay.me profile: <a className="text-link" href={razorpayLink} target="_blank" rel="noreferrer">razorpay.me/@harkiratsingh5450</a>. Share only after an order total is manually confirmed.</p>}<div className="policy-readiness">{checks.map(([key,label])=><span data-ready={Boolean(completed[key])} key={key}>{label}</span>)}</div><strong>Not ready for live checkout activation.</strong><p><Link className="text-link" href="/admin/policies">Edit policy settings</Link> · <Link className="text-link" href="/admin/delivery">Manage local delivery</Link></p></section>
  </main>;
}
