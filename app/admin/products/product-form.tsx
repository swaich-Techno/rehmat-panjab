...
export function AdminProductForm({ initial }: { initial?: ProductDraft }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...Object.fromEntries(form.entries()), id: initial?.id, featured: form.get("featured") === "on", character: String(form.get("character") ?? "").split(",").map((value) => value.trim()).filter(Boolean) };
    try {
