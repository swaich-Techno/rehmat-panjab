...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: variantRows }, { data: mediaRows },{data:bottleRows}] = await Promise.all([
    ...
    supabase.from("product_variants").select("id,size_ml,sku,price_paise,oil_cost_paise,enabled,bottle_id,inventory(quantity,reserved,low_stock_threshold)").eq("product_id", id).order("size_ml"),
    ...
    supabase.from("bottles").select("id,name,public_label,volume_ml").eq("status","active").order("display_order"),
  ]);
  if (!data) notFound();
  const variants = (variantRows ?? []).map((variant) => {
    ...
    return { id: variant.id, size_ml: Number(variant.size_ml), sku: variant.sku, price_paise: variant.price_paise, oil_cost_paise: variant.oil_cost_paise, enabled: variant.enabled,bottle_id:variant.bottle_id, quantity: inventory?.quantity ?? 0, reserved: inventory?.reserved ?? 0, low_stock_threshold: inventory?.low_stock_threshold ?? 2 };
  }) as VariantDraft[];
  const bottles=(bottleRows||[]).map(item=>({id:item.id,label:item.public_label||`${item.name} · ${Number(item.volume_ml)} ml`}));
  return <main id="main-content" className="admin-page"><p className="eyebrow">Administration · Edit product</p><h1>{data.name}</h1><AdminProductForm initial={data} /><AdminMediaManager productId={data.id} name={data.name} slug={data.slug} initial={(mediaRows??[]) as MediaDraft[]}/><AdminVariantManager productId={data.id} variants={variants} bottles={bottles}/></main>;
}
