...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: variantRows }] = await Promise.all([
    supabase.from("products").select("id,product_number,name,slug,status,subtitle,short_description,description,scent_family,scent_profile,image_path,campaign_image_path,featured,seo_title,seo_description,og_image_path").eq("id", id).maybeSingle(),
    supabase.from("product_variants").select("id,size_ml,sku,price_paise,enabled,inventory(quantity,reserved)").eq("product_id", id).order("size_ml"),
  ]);
  if (!data) notFound();
  const variants = (variantRows ?? []).map((variant) => {
    ...
    return { id: variant.id, size_ml: Number(variant.size_ml), sku: variant.sku, price_paise: variant.price_paise, enabled: variant.enabled, quantity: inventory?.quantity ?? 0, reserved: inventory?.reserved ?? 0 };
  }) as VariantDraft[];
