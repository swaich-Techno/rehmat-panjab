...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: variantRows }, { data: mediaRows },{data:bottleRows}] = await Promise.all([
    ...
    supabase.from("product_variants").select("id,size_ml,sku,price_paise,enabled,bottle_id,inventory(quantity,reserved,low_stock_threshold)").eq("product_id", id).order("size_ml"),
  ...
  const variants = (variantRows ?? []).map((variant) => {
    ...
    return { id: variant.id, size_ml: Number(variant.size_ml), sku: variant.sku, price_paise: variant.price_paise, enabled: variant.enabled,bottle_id:variant.bottle_id, quantity: inventory?.quantity ?? 0, reserved: inventory?.reserved ?? 0, low_stock_threshold: inventory?.low_stock_threshold ?? 2 };
  }) as VariantDraft[];
