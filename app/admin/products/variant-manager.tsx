...
export type VariantDraft = { id: string; size_ml: number; sku: string; price_paise: number | null; enabled: boolean; quantity: number; reserved: number };
...
function VariantForm({ productId, initial }: { productId: string; initial?: VariantDraft }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { id: initial?.id, productId, sizeMl: form.get("sizeMl"), sku: form.get("sku"), priceRupees: form.get("priceRupees"), quantity: form.get("quantity"), enabled: form.get("enabled") === "on" };
    try {
