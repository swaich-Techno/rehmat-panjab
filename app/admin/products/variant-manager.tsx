...
export type VariantDraft = { id: string; size_ml: number; sku: string; price_paise: number | null; oil_cost_paise: number | null; enabled: boolean; quantity: number; reserved: number; low_stock_threshold: number; bottle_id:string|null };
...
function VariantForm({ productId, initial,bottles }: { productId: string; initial?: VariantDraft;bottles:BottleOption[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { id: initial?.id, productId, sizeMl: form.get("sizeMl"), sku: form.get("sku"), priceRupees: form.get("priceRupees"), oilCostRupees: form.get("oilCostRupees"), quantity: form.get("quantity"), lowStockThreshold: form.get("lowStockThreshold"), bottleId:form.get("bottleId"), enabled: form.get("enabled") === "on" };
    try {
