...
export function WhatsAppOrder({ products, settings, compact = false }: { products: WhatsAppOrderProduct[]; settings: WhatsAppOrderSettings; compact?: boolean }) {
  ...
  function requestOrder() {
    ...
    const text = ["Hello Rehmat Panjab, I would like to request an order:", "", ...lines,...quotation,"",`Recipient: ${reviewedAddress.recipientName}`,`Mobile: ${reviewedAddress.mobile}`,"Delivery address:",...addressPreview(reviewedAddress).split("\n").slice(2),`PIN code: ${reviewedAddress.pinCode}`,"", "This is an order request. Rehmat Panjab will confirm stock, delivery availability, shipping charge and payment before accepting the order.", settings.defaultMessage].filter(Boolean).join("\n");
