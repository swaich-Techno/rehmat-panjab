...
export function WhatsAppOrder({ products, settings, compact = false }: { products: WhatsAppOrderProduct[]; settings: WhatsAppOrderSettings; compact?: boolean }) {
  ...
  const [quote, setQuote] = useState<{subtotalPaise:number;discountPaise:number;totalPaise:number;couponCode:string|null}|null>(null);
  ...
  async function applyCoupon(){
    ...
    const response=await fetch("/api/quote",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lines,couponCode:coupon})}); const data=await response.json().catch(()=>({message:"Coupon could not be checked."}));
  ...
  function requestOrder() {
    ...
    const quotation=quote?[`Normal subtotal: ${formatMoney(quote.subtotalPaise,"INR")}`,`Coupon: ${quote.couponCode}`,`Discount: ${formatMoney(quote.discountPaise,"INR")}`,`Final quoted total: ${formatMoney(quote.totalPaise,"INR")}`]:[];
