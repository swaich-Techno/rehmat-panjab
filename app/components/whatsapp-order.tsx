...
export function WhatsAppOrder({ products, settings, compact = false }: { products: WhatsAppOrderProduct[]; settings: WhatsAppOrderSettings; compact?: boolean }) {
  ...
  const [reviewedAddress,setReviewedAddress]=useState<DeliveryAddress|null>(null);
  ...
  async function applyCoupon(){
    ...
    const response=await fetch("/api/quote",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lines,couponCode:coupon,customerIdentifier:address.email||address.mobile||undefined,deliveryPin:address.pinCode||undefined})}); const data=await response.json().catch(()=>({message:"Coupon could not be checked."}));
  ...
  function updateAddress(value:DeliveryAddress){setAddress(value);setReviewedAddress(null);setQuote(null);}
  function reviewAddress(){const parsed=deliveryAddressSchema.safeParse(address);if(!parsed.success){setMessage(parsed.error.issues[0]?.message??"Check the delivery address.");return;}setAddress(parsed.data);setReviewedAddress(parsed.data);setMessage("Review the address below. You can correct it before opening WhatsApp.");}
  function requestOrder() {
    if(!reviewedAddress){setMessage("Review and confirm the complete delivery address first.");return;}
    ...
    const text = ["Hello Rehmat Panjab, I would like to request an order:", "", ...lines,...quotation,"",`Recipient: ${reviewedAddress.recipientName}`,`Mobile: ${reviewedAddress.mobile}`,"Delivery address:",...addressPreview(reviewedAddress).split("\n").slice(2),`PIN code: ${reviewedAddress.pinCode}`,"", "This is an order request. Rehmat Panjab will confirm inventory, delivery availability, shipping charge and payment before accepting the order.", settings.defaultMessage].filter(Boolean).join("\n");
  ...
  return <section className={compact ? "whatsapp-order is-compact" : "whatsapp-order"} aria-labelledby={`whatsapp-${available.map((item) => item.slug).join("-")}`}>
    ...
    <DeliveryAddressFields value={address} onChange={updateAddress}/>
    <button type="button" className="button button-outline" onClick={reviewAddress}>Review delivery address</button>
    {reviewedAddress&&<section className="address-preview" aria-live="polite"><h4>Confirm delivery address</h4><pre>{addressPreview(reviewedAddress)}</pre><p>Email is used for order service only and is not included in the WhatsApp message.</p><button type="button" className="text-link" onClick={()=>setReviewedAddress(null)}>Correct address</button></section>}
    ...
    <button className="button button-dark whatsapp-button" type="button" onClick={requestOrder} disabled={!reviewedAddress}>Open order request</button>
