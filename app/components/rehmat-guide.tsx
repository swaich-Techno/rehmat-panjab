...
export function RehmatGuide(){
  ...
  const panel=useRef<HTMLDivElement>(null),launcher=useRef<HTMLButtonElement>(null),field=useRef<HTMLInputElement>(null),contextProductId=useRef<string|undefined>(undefined);
  ...
  useEffect(()=>{const show=(event:Event)=>{const detail=(event as CustomEvent<{productId?:string;productName?:string}>).detail;contextProductId.current=detail?.productId;if(detail?.productName)setInput(`Tell me about ${detail.productName}`);setOpen(true);};window.addEventListener("open-rehmat-guide",show);return()=>window.removeEventListener("open-rehmat-guide",show);},[]);
  ...
  async function ask(text:string){const clean=text.trim();if(!clean||loading)return;setMessages(v=>[...v,{id:Date.now(),role:"visitor",text:clean}]);setInput("");setLoading(true);setError("");try{const response=await fetch("/api/rehmat-guide",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({message:clean,productId:contextProductId.current})});const value=await response.json() as GuideReply&{message:string};if(!response.ok)throw new Error(value.message);setMessages(v=>[...v,{id:Date.now()+1,role:"guide",text:value.message,products:value.products,layering:value.layering}]);}catch(cause){setError(cause instanceof Error?cause.message:"Please try again.");}finally{setLoading(false);}}
  function submit(event:FormEvent){event.preventDefault();void ask(input);}
  function whatsapp(products:GuideProduct[],layering=false){const lines=products.map(p=>`${p.name}: ${p.variants.map(v=>`${v.sizeMl} ml ${money(v.pricePaise)}`).join(" / ")}`);const number=String(config?.whatsapp.number??"").replace(/\D/g,"").slice(0,15);return `https://wa.me/${number}?text=${encodeURIComponent(`Sat Sri Akal, I would like to ask about ${layering?"this layering combination":"these fragrances"}:\n${lines.join("\n")}\nPlease confirm availability and ordering details.`)}`;}
  if(config&&!config.enabled)return null;
  return <>
    <button ref={launcher} className="guide-launcher" type="button" aria-label="Rehmat Guide is available" aria-expanded={open} aria-controls="rehmat-guide-panel" onClick={()=>setOpen(true)}><span aria-hidden="true">R</span><b>Ask Rehmat Guide</b><i aria-hidden="true"/></button>
    {open&&<div className="guide-shell" role="presentation"><button className="guide-scrim" aria-label="Close Rehmat Guide" onClick={()=>setOpen(false)}/><div ref={panel} id="rehmat-guide-panel" className="guide-panel" role="dialog" aria-modal="true" aria-labelledby="guide-title">
      <header><div><p>Rehmat Panjab</p><h2 id="guide-title">Rehmat Guide</h2><span>Scent guidance available</span></div><button type="button" aria-label="Close Rehmat Guide" onClick={()=>{setOpen(false);launcher.current?.focus();}}>×</button></header>
      ...
      {messages.length===1&&<div className="guide-prompts">{config?.prompts.slice(0,6).map(prompt=><button type="button" key={prompt} onClick={()=>void ask(prompt)}>{prompt}</button>)}</div>}
