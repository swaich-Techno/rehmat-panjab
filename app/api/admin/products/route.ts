...
const productSchema = z.object({
  ...
  status: z.enum(["draft", "coming_soon", "active", "sold_out", "archived"]), subtitle: z.string().trim().max(180).optional().default(""), shortDescription: z.string().trim().max(320).optional().default(""), description: z.string().trim().max(4000).optional().default(""), scentFamily: z.string().trim().max(80).optional().default(""), character: z.array(z.string().trim().min(1).max(40)).max(12).optional().default([]), suitableFor: z.array(z.string().trim().min(1).max(80)).max(16).optional().default([]), reviewsEnabled: z.boolean().optional().default(true), featured: z.boolean().optional().default(false), imagePath: assetPath, campaignImagePath: assetPath, seoTitle: z.string().trim().max(120).optional().default(""), seoDescription: z.string().trim().max(320).optional().default(""), ogImagePath: assetPath,
});

function toPayload(value: z.infer<typeof productSchema>) {
  return { product_number: value.productNumber, name: value.name, slug: value.slug, status: value.status, subtitle: value.subtitle, short_description: value.shortDescription, description: value.description, scent_family: value.scentFamily || null, scent_profile: { character: value.character }, occasions: value.suitableFor, reviews_enabled: value.reviewsEnabled, featured: value.featured, image_path: value.imagePath || null, campaign_image_path: value.campaignImagePath || null, seo_title: value.seoTitle || null, seo_description: value.seoDescription || null, og_image_path: value.ogImagePath || null };
}
