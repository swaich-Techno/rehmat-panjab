...
export async function POST(request:Request){
  ...
  const form=await request.formData(),file=form.get("file");if(!form.has("confirmed"))return NextResponse.json({message:"Confirm the exact product and media role before saving."},{status:400});const parsed=schema.safeParse({productId:form.get("productId"),role:form.get("role"),path:form.get("path"),alt:form.get("alt"),sortOrder:form.get("sortOrder"),generated:form.has("generated"),confirmMove:form.get("confirmMove")==="true"});if(!parsed.success)return NextResponse.json({message:parsed.error.issues[0]?.message??"Check the media fields."},{status:400});
