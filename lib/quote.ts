...
export async function calculateOrderQuote(lines:QuoteLine[],couponInput?:string,customerIdentifier?:string,deliveryPin?:string,testerPacks:TesterPackQuoteLine[]=[]):Promise<OrderQuote>{
  ...
  const {data:variants,error}=await admin.from("product_variants").select("id,product_id,price_paise,enabled,status,tester_pack_eligible,products!inner(status,tester_product_intake(content_approved,image_approved)),inventory(quantity,reserved),tester_variant_costs(margin_approved,packaging_approved)").in("id",ids);
  ...
  for(const pack of testerPacks){
    for(const variantId of pack.variantIds){
      const variant=variantsById.get(variantId); const product=variant&&(Array.isArray(variant.products)?variant.products[0]:variant.products); const intake=product&&(Array.isArray(product.tester_product_intake)?product.tester_product_intake[0]:product.tester_product_intake); const stock=variant&&(Array.isArray(variant.inventory)?variant.inventory[0]:variant.inventory); const costs=variant&&(Array.isArray(variant.tester_variant_costs)?variant.tester_variant_costs[0]:variant.tester_variant_costs);
      if(!variant||!variant.enabled||variant.status!=="active"||!variant.tester_pack_eligible||variant.price_paise===null||product?.status!=="active"||!intake?.content_approved||!intake.image_approved||!costs?.margin_approved||!costs?.packaging_approved)throw new Error("A selected tester is not approved for sale.");
  ...
  let discountPaise=testerPacks.reduce((saving,pack)=>{const unit=calculateTesterPackPrice(pack.variantIds.map(id=>variantsById.get(id)!.price_paise!),pack.packSize);return saving+unit.discountPaise*pack.quantity;},0); let promotionalLabel:string|null=testerPacks.length?"Build-your-own tester-pack saving":null; let autoCombinable=!testerPacks.length;
  ...
  for(const item of testerPacks.length?[]:discounts??[]){
  ...
  let couponCode=couponInput?.trim().toUpperCase()||null;
  ...
  if(couponCode){
    ...
    if(discountPaise&&!autoCombinable&&!coupon.combinable){ if(couponDiscount>discountPaise){discountPaise=couponDiscount;promotionalLabel=coupon.public_description;} else {couponId=null;couponCode=null;} }
