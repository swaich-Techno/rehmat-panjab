...
test("launch migration preserves inventory and records guarded operational state",async()=>{
  const migration=await read("supabase/migrations/202609100001_launch_contact_shipping_gulnaar.sql");
  ...
  assert.match(migration,/whatsapp_number='917009466475'/);assert.match(migration,/support_email_verified_at=null/);
...
test("customer links and policy safeguards use only the approved contact",async()=>{
  ...
  assert.match(current,/\+91 70094 66475/);assert.match(current,/917009466475/);assert.match(contact,/tel:\$\{merchant\.phoneE164\}/);assert.match(contact,/mailto:/);
